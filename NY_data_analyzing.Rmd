...
1. Number of Drug Overdose Deaths by Drug Categories in NYC
...
2. Percent of Drug Overdoes Deaths over Total Number of Deaths by Year in NYC: 
...
3. Number of Drug Overdose Deaths with Drug Categories by Year in NYC
...
4. Median Household Income: New York vs. The U.S
...
5. Income vs. Percent of Drug Overdose Death by Year in NYC
...
6. Map of Death rates by NYC counties:
...
7. Map of Population by NYC counties:

```{r}
add_text_county_pop = function(county_fips){
  data(county.map, package="choroplethrMaps", envir=environment())
  df = county.map[county.map$region %in% county_fips, ]
  #geom_text(data=df, aes(mean(long), mean(lat), label = paste0(str_to_title(pull(county_fips, county)), " County\n", pull(county_fips, death_rate))), color = "white")
  geom_label(data=df, aes(mean(long), mean(lat), label = paste0(str_to_title(pull(county_fips, county)), " County\nPopulation: ", pull(county_fips, population))), fill = "white", size = 3)
}
```
...
county_choropleth(ny_county_pop_2015, 
  ...
  highlight_county(ny_county_df_2015[which.max(pull(ny_county_df_2015, population)),]) +
  add_text_county_pop(ny_county_df_2015[which.max(pull(ny_county_df_2015, population)),])

# population is not really related to death rate by drug overdoes ?
```
