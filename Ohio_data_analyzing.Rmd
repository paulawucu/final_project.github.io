...
OH_income_df = 
  ...
  filter(str_detect(geography,"Ohio|United States"),
    year >= "2015") %>% 
  mutate(year = factor(year))
...
def = county.fips %>% 
  ...
  as.tibble()
def
oh_county_df = left_join(oh_county_df,def, by = "county") %>% 
...
highlight_county = function(county_fips)
{
  ...
  geom_polygon(data=df, aes(long, lat, group = group), color = "white", fill = NA, size = 1)
 # geom_text(data=df, aes(long, lat, label = pull(county_fips, county), color = "white"))
}
add_text_county = function(county_fips){
  data(county.map, package="choroplethrMaps", envir=environment())
  df = county.map[county.map$region %in% county_fips, ]
  geom_text(data=df, aes(mean(long), mean(lat), label = paste0(str_to_title(pull(county_fips, county)), " County\n", pull(county_fips, death_rate))), color = "yellow")
}
...
oh_county_df %>% 
  ...
  county_choropleth(state_zoom = c("ohio"),
                     legend = "Death Rate") +
 highlight_county(oh_county_df[which.max(pull(oh_county_df, death_rate)),])+
  add_text_county(oh_county_df[which.max(pull(oh_county_df, death_rate)),])
```